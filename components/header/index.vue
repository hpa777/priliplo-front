<template>
	<header>
		<div
			...
			<div
				class="flex justify-evenly sm:flex-grow-1 md:pt-2.5 lg:col-span-2 lg:row-start-1 lg:justify-between xl:col-span-1 xl:pt-4">
				<div
					class="flex flex-shrink-0 md:flex-col md:gap-y-[1.3rem] lg:flex-row lg:gap-x-10 xl:gap-x-[13.7rem]">
					<div class="flex flex-col gap-y-4">
						<HeaderCitySelect />
						<a class="text-center font-bold md:hidden" :href="'tel:' + $t('header_telephon')">{{
							$t("header_telephon")
						}}</a>
					</div>
					...
					<div
						...
						<button
							class="bg-my-yellow grid aspect-square h-10 place-content-center rounded-r-[0.31rem] md:h-full">
							<IconSearch class="h-6 w-6 md:h-4 md:w-4" />
						</button>
						<input
							type="text"
							placeholder="Поиск компании"
							class="flex-grow-1 indent-4 placeholder:text-stone-400 focus:outline-0" />
					</div>
				</div>
				...
				<div class="flex md:flex-col md:gap-y-[1.3rem] lg:flex-row lg:gap-x-10">
					<div class="hidden gap-x-1 md:flex">
						<a
							href=""
							class="grid aspect-square h-[1.56rem] place-content-center rounded-[0.31rem] bg-[#08c]">
							<IconTelegram />
						</a>
						<a
							href=""
							class="grid aspect-square h-[1.56rem] place-content-center rounded-[0.31rem] bg-[#34bf44]">
							<IconWhatsapp />
						</a>
					...
					<div class="flex sm:gap-x-6 md:gap-x-7">
						<button
							class="hidden flex-col items-center gap-y-1 sm:flex md:hidden"
							@click="showSearchMobile = true">
							<div
								class="bg-my-yellow grid aspect-square h-10 place-content-center rounded-[0.31rem] md:h-auto">
						...
						<HeaderRegisterBtn
							class="border-my-yellow hidden h-[1.56rem] items-center rounded-[0.31rem] border px-2.5 md:flex" />
					</div>
				</div>
			</div>

			<button
				class="bg-my-yellow grid aspect-square w-[3.8rem] place-content-center rounded-[0.31rem] md:mt-4 lg:hidden"
			...
			<nav
				...
				<HeaderRegisterBtn
					class="border-my-yellow flex w-fit items-center justify-center rounded-[0.31rem] border px-1 text-white sm:hidden" />
				<div
					id="search-form-2"
					class="flex h-[1.56rem] items-center overflow-hidden rounded-[0.31rem] border border-stone-400 bg-white sm:hidden">
...
const menuItemClick = (e: Event) => {
	...
	if (!el) return;
	if (!el.classList.contains("has-submenu") || el.classList.contains(clickedItemClass)) return true;
	e.preventDefault();
	closeAllSubmenu();
